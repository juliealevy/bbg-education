...
public class CourseCreateCommandHandlerTests
{
    ...
    public async void Handle_ShouldCreateNewCourse_WhenInputValid() {
        ...
        var savedCourse = CourseEntity.Create(1, command.Name, command.Description);
        _courseRepository.AddCourse(command.Name, command.Description).Returns((int)savedCourse.course_id!);
